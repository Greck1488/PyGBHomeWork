...
welcome_text = ('Я хочу сыграть с тобой в игру!\n'
                'Есть пики точеные, ой тоесть 2021 конфета\n'
                'Брать можно от 1 до 28 конфет за один ход\n'
                'Выигрывает тот, кто последним ходом заберет все конфеты.\n'
                'Приз? Кариес, вот твой приз.')
print(welcome_text)
    ...
    while candies_total > 0:
        if count == 0:
            ...
            if step > candies_total or step > max_take:
                step = int(input(
                    f'\nНапоминаю, не больше {max_take} конфет {lucky}, попробуй еще раз: '))
        ...
        if count == 1:
            ...
            if step > candies_total or step > max_take:
                step = int(input(
                    f'\nНапоминаю, не больше {max_take} конфет {loser}, попробуй еще раз: '))
            candies_total = candies_total - step
        if candies_total > 0:
            print(f'\nна кону еще {candies_total}')
            count = 0
        else:
            print('Баста, карапузик, кончились конфеты')
